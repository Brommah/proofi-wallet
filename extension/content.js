...
// --- Sign request relay using chrome.storage (robust) ---
window.addEventListener("__proofi_sign_request__",function(e){
  let detail=e.detail;
  if(!detail||!detail.message)return;
  console.log("[Proofi Extension] Sign request received, relaying via storage");

  // Write sign request to storage — popup will pick it up
  chrome.storage.local.set({
    proofi_sign_pending: { requestId: detail.requestId, message: detail.message, ts: Date.now() }
  });

  // Ask background to open popup
  chrome.runtime.sendMessage({type:"OPEN_SIGN_POPUP"});

  // Watch for sign result in storage
  function onStorageChange(changes, area) {
    if (area !== "local" || !changes.proofi_sign_result) return;
    let result = changes.proofi_sign_result.newValue;
    if (!result || result.requestId !== detail.requestId) return;

    // Clean up
    chrome.storage.onChanged.removeListener(onStorageChange);
    chrome.storage.local.remove(["proofi_sign_pending", "proofi_sign_result"]);

    // Send result back to page
    let responseDetail;
    if (result.error) {
      responseDetail = { requestId: detail.requestId, error: result.error };
    } else {
      responseDetail = { requestId: detail.requestId, signature: result.signature };
    }
    console.log("[Proofi Extension] Sign result received:", result.signature ? "success" : result.error);
  ...
  setTimeout(function() {
    ...
    window.postMessage({ type: "__proofi_sign_response__", detail: { requestId: detail.requestId, error: "Sign request timed out" } }, "*");
  }, 120000);
});
})();
