...
// UNUSED EXPORTS: BUTTON_POSITION, CERE_PROD_ENVS, LOGIN_PROVIDER, PAYMENT_PROVIDER, SUPPORTED_PAYMENT_NETWORK, TORUS_BUILD_ENV, TorusInpageProvider, WALLET_OPENLOGIN_VERIFIER_MAP, WALLET_VERIFIERS, preloadIframe
...
const CERE_PROD_ENVS = [TORUS_BUILD_ENV.CERE, TORUS_BUILD_ENV.CERE_AWS, TORUS_BUILD_ENV.CERE_LEGACY];
...
const utils_getTorusUrl = async (buildEnv, integrity) => {
  let torusUrl;
  ...
  switch (buildEnv) {
    ...
    case "cere-stage":
      torusUrl = `https://wallet.stage.cere.io?v${versionUsed}`;
      logLevel = "debug";
      break;
...
const UNSAFE_METHODS = ["eth_sendTransaction", "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4", "personal_sign", "eth_getEncryptionPublicKey", "eth_decrypt", "ed25519_transfer", "ed25519_sign", "ed25519_signRaw", "eth_transfer", "wallet_requestPermissions"];
...
class Torus {
  constructor() {
    ...
    defineProperty_default()(this, "loginOptions", {});

    defineProperty_default()(this, "useWalletConnect", void 0);

    defineProperty_default()(this, "popupMode", "popup");
  ...
  async init() {
    ...
    const {
      torusUrl,
      logLevel
    } = await utils_getTorusUrl(buildEnv, integrity);
  ...
  showWallet(path) {
    ...
    const showWalletHandler = chunk => {
      if (chunk.name === "show_wallet_instance") {
        ...
        const _chunk$data = chunk.data,
              {
          instanceId,
          target
        } = _chunk$data,
              extraParams = objectWithoutProperties_default()(_chunk$data, _excluded2);

        const finalUrl = new URL(`${this.torusUrl}/wallet${finalPath}`);
        Object.entries(embed_objectSpread(embed_objectSpread(embed_objectSpread({}, extraParams), params), {}, {
          instanceId
        })).forEach(_ref2 => {
    ...
    inpageProvider.tryPreopenHandle = (payload, cb) => {
      ...
      if (!Array.isArray(_payload) && UNSAFE_METHODS.includes(_payload.method)) {
        ...
        this._handleWindow(preopenInstanceId, {
          ...
          // Transaction popups should support `popupMode` option
...
;// CONCATENATED MODULE: ./src/preloadIframe.ts

const preloadIframe = async (buildEnv, version) => {
