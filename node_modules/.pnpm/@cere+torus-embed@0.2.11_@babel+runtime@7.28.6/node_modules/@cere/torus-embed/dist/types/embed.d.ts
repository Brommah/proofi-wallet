...
import { BUTTON_POSITION_TYPE, EMBED_TRANSLATION_ITEM, NetworkInterface, PAYMENT_PROVIDER_TYPE, PaymentParams, PopupMode, TorusCtorArgs, TorusLoginParams, TorusParams, TorusPublicKey, UserInfo, VerifierArgs, WALLET_PATH, WhiteLabelParams } from "./interfaces";
declare class Torus {
    ...
    private popupMode;
    constructor({ buttonPosition, buttonSize, modalZIndex, apiKey, popupMode, }?: TorusCtorArgs);
    init({ buildEnv, enableLogging, enabledVerifiers, network, loginConfig, showTorusButton, integrity, whiteLabel, skipTKey, useWalletConnect, mfaLevel, context, sessionId, sessionNamespace, popupMode, biconomy, authMethod, mode, }?: TorusParams): Promise<void>;
    ...
    showWallet(path: WALLET_PATH, params?: Record<string, string>, options?: {
        target?: string;
        onClose?: () => void;
    }): void;
    ...
    protected _handleWindow(preopenInstanceId: string, { url, target, features, popupMode, }?: {
        url?: string;
        target?: string;
        features?: string;
        popupMode?: PopupMode;
