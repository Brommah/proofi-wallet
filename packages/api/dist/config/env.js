...
function getEnvRequired(key, description) {
...
export const env = {
    ...
    MASTER_SECRET: getEnvRequired('PROOFI_MASTER_SECRET', 'Master secret for key derivation and HMAC-based OTP. Generate with: openssl rand -hex 32'),
    // Cere wallet passphrase — REQUIRED in production
    CERE_WALLET_PASSPHRASE: getEnvRequired('CERE_WALLET_PASSPHRASE', 'Passphrase for the Cere DDC wallet JSON file.'),
    // CORS — production whitelist + dev localhost
    CORS_ORIGINS: getEnv('CORS_ORIGINS', 'https://proofi-virid.vercel.app,https://proofi.ai,http://localhost:5173,http://localhost:5174,http://localhost:3000,http://localhost:8080'),
    ALLOWED_ORIGINS: getEnv('ALLOWED_ORIGINS', 'https://proofi-virid.vercel.app,https://proofi.ai,http://localhost:5173,http://localhost:5174,http://localhost:3000,http://localhost:8080'),
};
