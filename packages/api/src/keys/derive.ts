 ...
 */
export async function generateDerivationSalt(email: string): Promise<string> {
  ...
  const signature = await crypto.subtle.sign(
    'HMAC',
    key,
    encoder.encode(normalizedEmail + ':proofi-salt-v2'),
  );
 ...
 */
export async function deriveUserSeed(email: string): Promise<Uint8Array> {
  const encoder = new TextEncoder();
