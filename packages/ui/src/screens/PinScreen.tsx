...
export function PinScreen() {
  ...
  const [step, setStep] = useState<1 | 2>(1); // 1 = create, 2 = confirm
  const [localError, setLocalError] = useState<string | null>(null);
  const { email, setupPin, loading, error, clearError } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      // Create step - validate and move to confirm
      if (pin.length < 6) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full p-6">
      <div className="w-full max-w-sm">
        ...
        {/* Email badge */}
        <div className="flex items-center justify-center gap-2 rounded-lg bg-gray-900 border border-gray-800 px-3 py-2 mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-2">
              {step === 1 ? 'Enter a 6-8 digit PIN' : 'Re-enter your PIN to confirm'}
            </label>
            <input
              type="password"
              inputMode="numeric"
