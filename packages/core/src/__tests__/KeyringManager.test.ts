...
import type { KeyType, SigningPurpose } from '../types';
...
describe('KeyringManager', () => {
  ...
  describe('scoped signing / purposes', () => {
    ...
    it('stores and retrieves pairs by purpose', () => {
      mgr.derive({ type: 'ed25519', purposes: ['credential'] as SigningPurpose[] });
      mgr.derive({ type: 'ed25519', purposes: ['transaction'] as SigningPurpose[] });
      mgr.derive({ type: 'ed25519', purposes: ['credential', 'authentication'] as SigningPurpose[] });
  ...
  describe('error cases', () => {
    ...
    it('throws for unsupported key type on derive', () => {
      mgr.setMnemonic(TEST_MNEMONIC);
      expect(() => mgr.derive({ type: 'rsa' as KeyType })).toThrow('Unsupported key type');
    });

    it('throws for unsupported key type on import', () => {
      expect(() =>
        mgr.importKey({ type: 'rsa' as KeyType, secretKey: new Uint8Array(32) }),
